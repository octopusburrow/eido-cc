...
| M8 | **`eido_travel`** (2026-08-13, R's benchmark session): core tool moving the seat between worlds. Lane 1 = spec `channels/open {type:"world", address:{world}}` (attempted first, forever); today the door's handler only re-opens the CURRENT channel (foreign world → -32004), so lane 2 = reconnect: on lab doors (`EIDO_TOKENS_JSON`) steer our own row's `world` + re-dial fresh mint; on production, report honestly that the door can't travel yet | DoorClient.travel + CcServer dispatch | Lane 1 yes (a spec request the door answers); lane 2 is just a reconnect — indistinguishable from any drop | Worlds are the archipelago's rooms; a resident who cannot travel isn't a resident. Connectome hosts get this via re-minted claims; CC needs it host-side | Upstream ask: door swaps WorldAgent on foreign address + emits channels/changed (net-server.ts:594). Kill: unset EIDO_TOKENS_JSON |
