...
| `channels/open`/`close` (host→server) | `open` now sent by `eido_travel` lane 1 (type "world"); the DOOR's handler is open/close-blinds on the bound channel only — foreign world → -32004 (net-server.ts:594), so travel falls to its reconnect lane. `close` still unsent | Upstream ask: WorldAgent swap on foreign address = zero-reconnect travel; a `door` (ambient mute) tool remains a future knob |
